...
import { useAdminEvents, useDeleteEvent, useUpdateEvent } from '@/hooks/useEvents';
import { Button } from '@/components/ui/button';
import { Plus, Trash2, Eye, EyeOff, Pencil, Calendar } from 'lucide-react';
...
export default function AdminEvents() {
  ...
  const handleTogglePublish = async (event: typeof events[0]) => {
    try {
      await updateMutation.mutateAsync({
        ...
        gallery_image_paths: event.gallery_image_paths,
      });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage institute events, workshops, and dialogues.</p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  ...
                  <th className="text-left text-xs font-medium text-gray-500 uppercase px-6 py-3 hidden md:table-cell">Location</th>
                ...
                {events.map((event) => (
                  <tr key={event.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      {event.speaker && <p className="text-xs text-gray-400 mt-0.5">Speaker: {event.speaker}</p>}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        {event.is_published ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
